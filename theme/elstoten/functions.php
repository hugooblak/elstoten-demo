<?php
/**
 * Elstöten theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Elstöten Leads" plugin, so leads survive a theme change.
 *
 * @package Elstöten
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'elstoten', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'elstoten' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'elstoten' ) ) );

		register_block_pattern_category( 'elstoten-sections', array( 'label' => __( 'Elstöten sections', 'elstoten' ) ) );
		register_block_pattern_category( 'elstoten-pages', array( 'label' => __( 'Elstöten page layouts', 'elstoten' ) ) );
	}
);
