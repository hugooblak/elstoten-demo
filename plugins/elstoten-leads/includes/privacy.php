...
add_filter(
	...
	function ( $exporters ) {
		$exporters['elstoten-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'elstoten-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'elstoten-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'elstoten-leads',
						'group_label' => __( 'Quote requests', 'elstoten-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['elstoten-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'elstoten-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
