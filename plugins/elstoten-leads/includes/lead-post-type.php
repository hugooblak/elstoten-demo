...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'elstoten-leads' ),
					'singular_name' => __( 'Lead', 'elstoten-leads' ),
					'edit_item'     => __( 'Förfrågan', 'elstoten-leads' ),
					'search_items'  => __( 'Search leads', 'elstoten-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'elstoten-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'elstoten-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'elstoten-leads' ),
			'npl_phone'    => __( 'Phone', 'elstoten-leads' ),
			'npl_service'  => __( 'Needs', 'elstoten-leads' ),
			'npl_timeline' => __( 'When', 'elstoten-leads' ),
			'npl_zip'      => __( 'ZIP', 'elstoten-leads' ),
			'npl_source'   => __( 'Source', 'elstoten-leads' ),
			'npl_status'   => __( 'Status', 'elstoten-leads' ),
			'date'         => __( 'Received', 'elstoten-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'elstoten-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'elstoten-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'elstoten-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'elstoten-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'elstoten-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'elstoten-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'elstoten-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'elstoten-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'elstoten-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'elstoten-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'elstoten-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'elstoten-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
